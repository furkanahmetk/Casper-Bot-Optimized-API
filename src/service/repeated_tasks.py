...
class Repeated_Task():
    ...
    def data_cron(self):
        ...
        current_validator_public_keys = [item['public_key'] for item in validator_list]
        current_validator_dict = {item['public_key']: item for item in validator_list}
        ...
        validators_in_db = self.validator.find({"public_key": {"$in": current_validator_public_keys}})
        ...
        update_validators = list(set(validators_in_db_public_keys).intersection(set(current_validator_public_keys)))
        #Find which validators are new and an entry should created in the database
        insert_validators = list(set(current_validator_public_keys)-set(validators_in_db_public_keys))
       
        if len(insert_validators) > 0:
            self.validator.create_many([current_validator_dict[x] for x in insert_validators])
